...
#[acmd_script( agent = "richter", script = "game_final_start" , category = ACMD_GAME )]
unsafe fn richter_final_start_game_original(fighter: &mut L2CAgentBase) {
    ...
    if is_excute(fighter) {
        ...
        PostureModule::set_scale(fighter.module_accessor, 5.0, false);
    ...
    if is_excute(fighter) {
        ArticleModule::set_visibility_whole(fighter.module_accessor, *FIGHTER_SIMON_GENERATE_ARTICLE_COFFIN, true,ArticleOperationTarget(*ARTICLE_OPE_TARGET_LAST));
        CAM_ZOOM_OUT(fighter);
    }
    if is_excute(fighter) {
        ATTACK(fighter, 0, 0, Hash40::new("top"), 3.0, 80, 100, 80, 0, 22.0, 0.0, 18.0, 30.0, None, None, None, 0.0, 0.0, *ATTACK_SETOFF_KIND_OFF, *ATTACK_LR_CHECK_SPEED, false, f32::NAN, 0.0, 0, false, false, false, true, true, *COLLISION_SITUATION_MASK_GA, *COLLISION_CATEGORY_MASK_FIGHTER, *COLLISION_PART_MASK_ALL, false, Hash40::new("collision_attr_normal"), *ATTACK_SOUND_LEVEL_L, *COLLISION_SOUND_ATTR_PUNCH, *ATTACK_REGION_NONE);
