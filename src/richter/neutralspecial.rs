...
pub const CHECK_FRAME: f32 = 10.0;
pub const SPAWN_FRAME: f32 = 25.0;
...
unsafe fn richter_special_n_game(fighter: &mut L2CAgentBase) {
    ...
    if is_excute(fighter) && canspawn  {
        ...
        WorkModule::on_flag(boma, *FIGHTER_SIMON_INSTANCE_WORK_ID_FLAG_CROSS);
    }
    frame(lua_state, SPAWN_FRAME+1.0);
    if is_excute(fighter) && canspawn  {
        //
    }
}
...
unsafe fn richter_special_n_effect(fighter: &mut L2CAgentBase) {
    ...
    if is_excute(fighter) {
        EFFECT_FOLLOW(fighter, Hash40::new("sys_genesis_start"), Hash40::new("top"), 0, 6, -3, 0, 0, 0, 0.20, true);
    }
